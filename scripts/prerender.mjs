...
<p style="color:rgba(234,241,248,.72);font-size:16px;line-height:1.6;margin-top:12px">${FROM_PRICE
  ? `<strong style="color:#EAF1F8">Book and pay for a guaranteed parking space in Belfast from ${FROM_PRICE} ${FROM_UNIT}</strong>${FROM_HOURS ? ` (${FROM_HOURS})` : ''} &mdash; reserved in advance at school, church and GAA club car parks, and held for you when you arrive. ParkEasy also lists ${NETWORK.spots} free spots, on-street bays and local parking tips right across Northern Ireland, the Republic and Britain.`
  : `<strong style="color:#EAF1F8">${NETWORK.spots} parking spots across Northern Ireland, the Republic and Britain &mdash; including ${NETWORK.gems} hidden gems the locals use and ${NETWORK.ev} EV chargers.</strong> Know where you're parking, what it costs and what the restrictions are before you leave the house, in Belfast, Derry~Londonderry, Lisburn, Newry, Bangor and towns right across Northern Ireland.`}</p>
