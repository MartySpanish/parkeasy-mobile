...
// One function for both pass operations (Hobby plan caps 12 functions).
// POST /api/passes with body { action: 'buy'|'redeem', ... }.
export default async function handler(req, res) {
  ...
  if (typeof body === 'string') { try { body = JSON.parse(body); } catch { body = {}; } }
  req.body = body;
  const action = body?.action || String(req.query?.action || '');
